...
-file("src/egret_lexer.xrl", 10).
yyaction_0(TokenChars, TokenLine) ->
     { token, validate_real (TokenLine, TokenChars) } .

-compile({inline,yyaction_1/0}).
-file("src/egret_lexer.xrl", 11).
yyaction_1() ->
     skip_token .

-compile({inline,yyaction_2/0}).
-file("src/egret_lexer.xrl", 12).
yyaction_2() ->
     skip_token .
